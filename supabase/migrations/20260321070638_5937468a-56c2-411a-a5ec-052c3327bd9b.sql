...
BEGIN
  ...
  -- Skip individual audio tracks — earnings are calculated from the mixed track only
  IF TG_TABLE_NAME = 'voice_recordings' AND NEW.recording_type = 'individual' THEN
    RETURN NEW;
  END IF;
  ...
  -- Calculate payment amount per participant
  ...
  -- *** For mixed audio: credit ALL participants in the session ***
  IF TG_TABLE_NAME = 'voice_recordings' AND NEW.recording_type = 'mixed' AND NEW.session_id IS NOT NULL THEN
    FOR v_participant IN
      SELECT DISTINCT user_id
      FROM public.voice_recordings
      WHERE session_id = NEW.session_id
        AND recording_type = 'individual'
        AND user_id IS NOT NULL
    LOOP
      -- Check duplicate per participant
      IF NOT EXISTS (
        SELECT 1 FROM public.earnings_ledger
        WHERE submission_id = NEW.id AND user_id = v_participant.user_id AND entry_type = 'task_payment'
      ) THEN
        INSERT INTO public.earnings_ledger (
          ...
          v_participant.user_id, NEW.campaign_id, NEW.id, v_submission_type,
          'task_payment', v_amount, v_currency, 'credited',
          'Auto-credited: ' || v_submission_type || ' session approved'
        )
        RETURNING id INTO v_earning_id;

        -- Referral cascade for this participant
        PERFORM _process_referral_cascade(
          v_earning_id, v_participant.user_id, NEW.campaign_id,
          NEW.id, v_submission_type, v_amount, v_currency
        );
      END IF;
    END LOOP;
  ...
  -- *** Non-audio or solo audio: original single-user logic ***
  IF EXISTS (
    SELECT 1 FROM public.earnings_ledger
  ...
  -- Referral cascade for single user
  PERFORM _process_referral_cascade(
    v_earning_id, NEW.user_id, NEW.campaign_id,
    NEW.id, v_submission_type, v_amount, v_currency
  );
...
-- Helper function for referral cascade (avoids code duplication)
CREATE OR REPLACE FUNCTION public._process_referral_cascade(
  p_earning_id uuid,
  p_user_id uuid,
  p_campaign_id uuid,
...
BEGIN
  ...
  IF v_ref_config IS NULL THEN RETURN; END IF;
  ...
  ELSE
    v_pool_amount := p_amount * (v_ref_config.pool_percent / 100.0);
  END IF;

  IF v_pool_amount <= 0 THEN RETURN; END IF;
  ...
  FOR v_lvl IN 1..LEAST(v_ref_config.max_levels, 5) LOOP
    ...
    INSERT INTO public.earnings_ledger (
      ...
      v_level_user, p_campaign_id, p_submission_id, p_submission_type,
      'referral_bonus', v_level_bonus, p_currency, 'credited',
      'Referral L' || v_lvl || ' bonus from ' || p_submission_type,
      p_earning_id
    );
