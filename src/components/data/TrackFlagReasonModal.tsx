...
interface TrackFlagReasonModalProps {
  open: boolean;
  onClose: () => void;
...
export function TrackFlagReasonModal({ open, onClose, onConfirm, trackLabel }: TrackFlagReasonModalProps) {
      ...
      <DialogContent className="bg-[hsl(240_6%_10%)] border-white/10 text-white max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-amber-400">
            <Flag className="h-5 w-5" /> Flag Track Individual
          </DialogTitle>
          {trackLabel && (
            <p className="text-[13px] text-white/40 mt-1">Track: {trackLabel}</p>
          )}
        </DialogHeader>

        <div className="flex flex-col gap-2 py-2">
          {TRACK_FLAG_REASONS.map((reason) => (
            <button
              key={reason}
