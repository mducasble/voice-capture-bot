...
serve(async (req) => {
  ...
  try {
    ...
    await supabase
      ...
      .eq('status', 'processing')
      .eq('job_type', 'enhance')
      .lt('started_at', new Date(now - 10 * 60 * 1000).toISOString());

    // 3. Claim ONE pending job (highest priority, oldest first)
    const { data: jobs, error: fetchErr } = await supabase
      .from('analysis_queue')
      .select('id, recording_id, attempts, max_attempts, job_type, current_segment, total_segments, segment_data')
      .eq('status', 'pending')
    ...
    const { error: claimErr } = await supabase
      ...
      .update({
        ...
        attempts: job.current_segment > 0 ? job.attempts : job.attempts + 1,
    ...
    // 6. Route based on job_type
    const baseUrl = Deno.env.get('SUPABASE_URL');
    const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
    const jobType = job.job_type || 'analyze';

    if (jobType === 'enhance') {
      // Build payload for enhance-audio
      const enhancePayload: any = {
        recording_id: rec.id,
        job_id: job.id,
      };
      ...
      if (job.current_segment > 0 && job.segment_data) {
        ...
        console.log(`🔄 Enhance job ${job.id} rec=${rec.id} init`);
      }
      ...
      try {
        ...
        if (!resp.ok || respText.includes('<!DOCTYPE html>')) {
          ...
          // For segment continuation, don't increment attempts aggressively
          const effectiveAttempts = job.current_segment > 0 ? job.attempts : job.attempts + 1;
          const newStatus = effectiveAttempts >= job.max_attempts ? 'failed' : 'pending';
          await supabase
            .from('analysis_queue')
            .update({
              status: newStatus,
            ...
            .eq('id', job.id);
          console.error(`❌ Enhance job ${job.id}: ${errorMsg}`);
        } else {
          // enhance-audio handles its own status updates for multi-segment
          // For single-shot (done in one call), it also marks done
          console.log(`✓ Enhance job ${job.id} rec=${rec.id} step completed`);
        }
      } catch (err) {
